...
不同的字典类型都是使用键和值构成的。但是值也可以是字典类型。可以定义一个bug报告系统，以方便用户（customer）查看bug。例如：
...
report本身是一个结构图，同时他的owner的值是Customer结构图。当然可以使用`.`访问结构图的值：
...
可是，现在的customer投诉说，公司的名称不正确即company的值不对，因此我们需要更新这个嵌套结构图：
